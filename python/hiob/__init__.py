...
class HIOB:
	def __init__(
		...
		balance_regression_factor: float = None,
...
class StochasticHIOB:
	...
	def from_h5_file(
		...
		ransac_pairs_per_bit: int = None,
		ransac_sub_sample: int = None,
		itq_central: bool = None,
		itq_iterations: int = None,
		update_parallel: bool = None,
		displace_parallel: bool = None,
		...
		self._rust_shiob = globals()[specific_type](
			...
			center_biases=None if center_biases is None else center_biases.astype(self._input_type),
			balance_regression_factor=balance_regression_factor,
			init_greedy=init_greedy,
			init_ransac=init_ransac,
			init_itq=init_itq,
			ransac_pairs_per_bit=ransac_pairs_per_bit,
			ransac_sub_sample=ransac_sub_sample,
	...
	def from_ndarray(
		...
		return self
	def _post_constructor_init(self, specific_type):
		attributes = {}
...
class MinHashSearcher:
	def __init__(
		...
		output_type_name = _bits_type_name(self._output_type)
		# Create specific instance
		specific_type = "RawMinHashSearcher_{:}_{:}".format(input_type_name, output_type_name)
